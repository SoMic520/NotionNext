...
export default async function getPageProperties(
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    if (!excludeProperties.includes(schemaType)) {
      ...
      switch (schemaType) {
        ...
        case 'person': {
          ...
          for (let i = 0; i < rawUsers.length; i++) {
            ...
            const userKey = Array.isArray(userId) ? userId[1] : userId

            if (!userKey || resolvedUserIds.has(userKey)) {
              continue
            }

            resolvedUserIds.add(userKey)
            const user = await getCachedNotionUser(userId, userKey)
            if (user) {
              users.push(user)
            }
          }
...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  const slugSource = postProperties.slug ?? postProperties.id
  const publishDate = postProperties?.publishDay
    ? new Date(postProperties.publishDay)
    : null
  const isValidPublishDate = publishDate instanceof Date && !isNaN(publishDate)
  const publishYear = isValidPublishDate
    ? publishDate.getUTCFullYear()
    : null
  const publishMonth = isValidPublishDate
    ? String(publishDate.getUTCMonth() + 1).padStart(2, 0)
    : null
  const publishDay = isValidPublishDate
    ? String(publishDate.getUTCDate()).padStart(2, 0)
    : null
  let fullPrefix = ''
  ...
  allSlugPatterns.forEach((pattern, idx) => {
    if (pattern === '%year%' && publishYear !== null) {
      fullPrefix += publishYear
    } else if (pattern === '%month%' && publishMonth !== null) {
      fullPrefix += publishMonth
    } else if (pattern === '%day%' && publishDay !== null) {
      fullPrefix += publishDay
    } else if (pattern === '%slug%') {
      fullPrefix += slugSource
  ...
  if (fullPrefix) {
    return `${fullPrefix}/${slugSource}`
  } else {
    return `${slugSource}`
  }
}

async function getCachedNotionUser(userId, cacheKey) {
  const resolvedCacheKey = cacheKey ?? (Array.isArray(userId) ? userId[1] : userId)
  if (!resolvedCacheKey) {
    return null
  }

  if (notionUserCache.has(resolvedCacheKey)) {
    return notionUserCache.get(resolvedCacheKey)
  }

  const fetchPromise = notionAPI
    .getUsers(userId)
    .then(res => {
      const resValue =
        res?.recordMapWithRoles?.notion_user?.[resolvedCacheKey]?.value
    ...
    .catch(error => {
      console.warn('Failed to fetch Notion user', resolvedCacheKey, error)
      return null
    })

  notionUserCache.set(resolvedCacheKey, fetchPromise)
  return fetchPromise
}
