...
/**
 * 验证外部资源URL是否合法
 * 仅允许 http(s) 协议和相对路径，防止 javascript: 等协议注入
 * @param {string} url
 * @returns {boolean}
 */
function isValidResourceUrl(url) {
  ...
  return trimmed.startsWith('/') || trimmed.startsWith('https://') || trimmed.startsWith('http://') || trimmed.startsWith('//')
}
...
export function loadExternalResource(url, type = 'js') {
  ...
  const elements =
    type === 'js'
      ? document.querySelectorAll(`script[src='${CSS.escape(url)}']`)
      : document.querySelectorAll(`link[href='${CSS.escape(url)}']`)
