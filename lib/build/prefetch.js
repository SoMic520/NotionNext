...
function normalizePageList(allPages) {
...
export function getPriorityPages(allPages) {
  const published = normalizePageList(allPages).filter(
  ...
  const top5Latest = [...published]
    .sort((a, b) => {
      const dateA = new Date(a?.publishDate || 0).getTime() || 0
      const dateB = new Date(b?.publishDate || 0).getTime() || 0
      return dateB - dateA
    })
    .slice(0, 5)
...
async function doPrefetch(allPages, concurrency = 8) {
  const pages = normalizePageList(allPages)
  const limit = pLimit(Math.max(1, Number(concurrency) || 8))
  let hit = 0
  let fetched = 0
  let failed = 0

  console.log(
    `[Prefetch][pid:${process.pid}] start ${pages.length} page blocks`
  )
