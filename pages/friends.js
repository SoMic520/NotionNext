...
import { getFriendsAndCategories } from '@/lib/notion-friends'

export async function getStaticProps() {
  let data = [], categories = [], debug = { ok: true }
  try {
    const r = await getFriendsAndCategories()
    data = r.items
    categories = r.categories
    if (!data.length) debug = { ok: false, reason: 'EMPTY_OR_FILTERED' }
  } catch (e) {
    debug = { ok: false, reason: 'QUERY_ERROR', message: String(e?.message || e) }
  }
  return { props: { data, categories, debug }, revalidate: 3600 }
}

export default function FriendsPage({ data = [], categories = [], debug }) {
  const groups = categories.map(cat => ({
    cat,
    items: data
      .filter(f => (cat === '未分类' ? !f.Category : f.Category === cat))
    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      {isEmpty ? (
        <div className="text-sm opacity-70">
          暂无友链。请在 Notion 库中添加至少一条 <b>Status=正常</b> 的记录。
        </div>
      ) : (
                  ...
                  {items.map(item => (
                    <li key={item.URL} className="border rounded-2xl p-4 hover:shadow transition">
                      ...
                      <div className="mt-3 text-xs opacity-60">
                        {item.RSS ? <a className="underline" href={item.RSS} target="_blank" rel="noreferrer">RSS</a> : ' '}
                      </div>
                    </li>
                  ))}
                </ul>
