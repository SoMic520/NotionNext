    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      {!debug.ok && (
        <div className="mb-6 text-sm p-3 rounded-lg border">
          ...
          {debug.message && (
            <pre className="mt-2 text-xs whitespace-pre-wrap opacity-70">{debug.message}</pre>
          )}
          <div className="mt-2 text-xs opacity-70">
            自检：打开 <a className="underline" href="/api/friends-test">/api/friends-test</a>，两个值都应为 ✅ set。
          </div>
        </div>
      )}
                  ...
                  {items.map(item => (
                    <li key={item.URL} className="border rounded-2xl p-4 hover:shadow transition">
                      ...
                      <div className="mt-3 text-xs opacity-60">
                        {item.RSS ? (
                          <a className="underline" href={item.RSS} target="_blank" rel="noreferrer">
                            RSS
                          </a>
                        ) : (
                          ' '
